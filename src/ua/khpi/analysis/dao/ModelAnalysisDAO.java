...
public class ModelAnalysisDAO {
	...
	public void insertAnalysisResults(String map, Artifact artifact, Timestamp timestamp) {
		String insertSql = "insert into ea_prm(prm_map, prm_timestamp, prm_year, prm_month, prm_day, prm_proc, prm_type, prm_indeg, prm_outdeg, prm_centr, prm_rank, prm_pc, prm_opt) "
				+ "values (:prm_map, :prm_timestamp, :prm_year, :prm_month, :prm_day, :prm_proc, :prm_type, :prm_indeg, :prm_outdeg, :prm_centr, :prm_rank, :prm_pc, :prm_opt)";
		...
		try (Connection con = sql2o.open()) {
			con.createQuery(insertSql).addParameter("prm_map", map).addParameter("prm_year", Year.now().getValue())
					...
					.addParameter("prm_pc", artifact.getCost()).addParameter("prm_opt", artifact.getOpt())
					.executeUpdate();
		}
	}
